...
// 		System.out.println(Arrays.deepToString(temp));
	...
	public static void RankOfMatrix() {
		...
		//diagonalMatrix
		List diagonal1=new ArrayList();
		List diagonal2=new ArrayList();
		for(int i=0;i<rankOfMatrix.length;i++) {
			for(int j=0;j<rankOfMatrix.length;j++) {
				if(i==j)
					diagonal1.add(rankOfMatrix[i][j]);
				if(j==(rankOfMatrix.length-1)-i)
					diagonal2.add(rankOfMatrix[i][j]);
			}
		}
